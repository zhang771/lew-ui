...
function warning(e: MessageFnOptions) {
  message({
    type: 'warning',
    e,
  })
...
function error(e: MessageFnOptions) {
  message({
    type: 'error',
    e,
  })
...
function info(e: MessageFnOptions) {
  message({
    type: 'info',
    e,
  })
...
function normal(e: MessageFnOptions) {
  message({
    type: 'normal',
    e,
  })
...
function success(e: MessageFnOptions) {
  message({
    type: 'success',
    e,
  })
...
function loading(e: MessageFnOptions) {
  message({
    type: 'loading',
    e,
  })
...
function clear(id?: number) {
  ...
  if (id) {
    ...
    setTimeout(() => {
      if (messageDom)
        LewMessageDom?.removeChild(messageDom)
    }, 120)
  }
...
function message({ type, e }: MessageOptions) {
...
function setMessage({ type, e }: MessageOptions) {
  const LewMessageDom = document.getElementById('lew-message')
